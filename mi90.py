...
with torch.no_grad():
    ...
    data = torch.load(data_root+'train_data_'+shot+'shot.pt')
    images = data['image']
    '''
    ...
    image_features = torch.load('img_embedding/train_'+shot+'shot.pt')
    text_features = torch.load('txt_embedding/'+'all.pt')
...
np.save('mutual_info_'+shot+'shot.npy',mutual_info)
