...
def main(shot,num_concept,epoch):
    ...
    tr_att_activations = torch.Tensor(np.load('save_des_'+shot + '/activation_train_'+shot+'shot'+str(num_concept)+epoch+'.npy'))
    t_att_activations = torch.Tensor(np.load('save_des_'+shot + '/activation_test_'+shot+'shot'+str(num_concept)+epoch+'.npy'))

    cls_truth = np.load('class_label_des_train_'+str(num_concept)+'.npy')
