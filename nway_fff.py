...
def main():
    ...
    #cls_truth = np.load('class_label_des_xx2.npy')
    #print(cls_truth.shape)

    # primitive concept activations
    tr_att_activations = torch.Tensor(np.load('save_des' + "/attribute_activations_train_pt.npy"))
    t_att_activations = torch.Tensor(np.load('save_des' + "/attribute_activations_valid_pt.npy"))
    #tr_att_activations /= tr_att_activations.mean(dim=-1,keepdim=True)
    #t_att_activations /= t_att_activations.mean(dim=-1,keepdim=True)
    #scale = 10
    #tr_att_activations *= scale
    #t_att_activations *=scale
    #cls_truth = np.zeros((200,t_att_activations.shape[1]))
    cls_truth = np.load('class_label_des90_400.npy')
    #tr_ground_truth = cls_truth[train_y]
    #t_ground_truth = cls_truth[test_y]
    tr_ground_truth = np.load('ground_truth_train.npy')
    t_ground_truth = np.load('ground_truth_valid.npy')
